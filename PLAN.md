...
- ✅ Spatie-style roles + permissions: V16 creates `roles`, `permissions`, `role_has_permissions`, `user_has_roles` tables with seeded roles (admin, bookkeeper, accountant, client) and 17 permissions. `Role` + `Permission` entities with `@ManyToMany`. `User.role` string replaced with `@ManyToMany Set<Role>` + Spatie-style helpers (`hasRole`, `hasPermission`, `allPermissions`). `SecurityConfig` builds Spring Security authorities from both roles (`ROLE_admin`) and permissions (`view-dashboard`) so both `hasRole()` and `hasAuthority()` work. `thymeleaf-extras-springsecurity6` added for `sec:authorize` in templates.
- ✅ Email-based user invitations: V17 adds `invite_token` + `invite_sent_at` to users. Admin `/admin/users` index with invite/edit/resend/enable-disable/delete. Invite flow: admin enters email+name+roles → system generates UUID token, creates user with placeholder password, sends branded invite email with CTA → `GET /invite/{token}` renders public set-password page → `POST` hashes password, clears token, redirects to login. Sidebar gains "Users" link under Admin section (visible only with `manage-users` permission).
